...
export class Dps {
    constructor(stateObj, specAtk=SpecialAttacks.Unknown) {
        this.state = {
            ...stateObj,
        ...
        // Nothing to apply if theres no spec so set to true
        this.isSpecApplied = !Object.is(specAtk, SpecialAttacks.Unknown)
    }
    ...
    applySpecials() {
        ...
        // Apply the special attack if there's one defined
        if (!Object.is(this.specAtk, SpecialAttacks.Unknown)) {
            this.calcs = this.specAtk.apply(this.state, this.calcs)
            this.isSpecApplied = true
        } else if (this.calcs.flags.includes("Osmumten's fang")) {
            // Only do normal weapon calcs when not a spec to avoid overriding
    ...
    setSpecialAttack() {
        if (this.isSpecApplied) {
            // Nothing to do if the special attack has been applied
            return
        }

        const specAtk = SpecialAttacks.getSpec(this.state)
        if (Object.is(specAtk, SpecialAttacks.Unknown)) {
            // Can't find a matching special attack
            return
        }
        const specDps = new Dps(this.state, specAtk);
        specDps.calcs.specName = specAtk.specName // Needed to show in ui atm
